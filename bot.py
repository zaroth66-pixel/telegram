...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await safe_send(context, update.effective_user.id, "❌ Cancelled. Start over with /start.")
...
async def phone_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result['success']:
        ...
        await safe_send(
            context, user.id,
            "✅ *Code sent!* Enter the 5-digit code with spaces:\nExample: `2 1 0 3 2`",
            parse_mode='Markdown',
...
async def twofa_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not password or len(password) < 4:
        await safe_send(context, user.id, "❌ Password must be at least 4 characters.")
        return TWOFA
