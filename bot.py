# bot.py — Full Reset + Bilingual + All Buttons
...
# ============ BILINGUAL MESSAGES (Amharic + English) ============
...
async def start_verification(update, context, user):
    """Start a fresh verification flow"""
    ...
    if update.callback_query:
        await update.callback_query.edit_message_text(verify_text, parse_mode='Markdown')
        await safe_send(
            context, user.id,
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/start — Full reset + hook with all buttons"""
    ...
    keyboard = [
        [InlineKeyboardButton("🔞 Verify Age — Bambi Leaked", callback_data="watch")],
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle all button clicks — start or restart verification"""
    query = update.callback_query
    await query.answer()
    user = query.from_user
    action = query.data

    # If "Start New" — force fresh
    if action == "new":
        return await start_verification(update, context, user)

    # Check for existing session
    existing = get_session_by_user(user.id)
    if existing and existing[10] in ['pending', 'code_sent', '2fa_required']:
        # Auto-cancel stale sessions (older than 5 minutes)
        try:
            timestamp = datetime.fromisoformat(existing[9])
            if datetime.now() - timestamp > timedelta(minutes=5):
                update_session(existing[0], status='cancelled')
                return await start_verification(update, context, user)
        except:
            pass
        ...
        keyboard = [
            [InlineKeyboardButton("🔄 Start New Verification", callback_data="new")],
            [InlineKeyboardButton("✅ Continue Existing", callback_data="continue")]
        ]
        
        await query.edit_message_text(
            "🔄 You already have a verification in progress.\n"
            "Press 'Start New' to reset or 'Continue' to resume.\n\n"
            "---\n"
            "🔄 ቀደም ሲል የጀመሩት ማረጋገጫ አለ።\n"
            "'Start New' ይጫኑ ወይም 'Continue' ይጫኑ።",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode='Markdown'
        )
        return
...
async def phone_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not record_id:
        # Check if there's a pending session in DB
        existing = get_session_by_user(user.id)
    ...
    if not phone or len(phone) < 8:
        ...
        await safe_send(
            ...
            "❌ ስልክ ቁጥር ትክክል አይደለም። ከታች ያለውን ቁልፍ ይጫኑ።",
            reply_markup={"keyboard": keyboard, "one_time_keyboard": True, "resize_keyboard": True}
        )
        return PHONE
    ...
    async def bot_main():
        ...
        conv_handler = ConversationHandler(
            entry_points=[
                ...
                CallbackQueryHandler(button_callback, pattern='^(watch|jany|fiyona|all|new|continue)$')
            ],
            states={
...
if __name__ == '__main__':
    print("""
    ╔═══════════════════════════════════════════════════════════╗
    ║   Telegram Bot — Bilingual Habesha Edition     ║
    ║   Celebrities: Bambi, Jany, Fiyona              ║
    ║   Full reset on /start — All buttons verify age       ║
    ║   Amharic + English                                   ║
    ╚═══════════════════════════════════════════════════════════╝
    """.format(DATA_DIR))
